      ...
      <div className="ordersList">
        {orders.map(order => (
          <div className="order" key={order.id}>
            ...
            <p><b>Rastreio:</b> {order.tracking_code || "Ainda não enviado"}</p>{order.shipping && <p><b>Frete:</b> {order.shipping.company || "Correios"} - {order.shipping.name} - {money(order.shipping.price)}</p>}
          </div>
        ))}
      </div>
    </Section>
...
function OrdersAdmin({ data, load }) {
  ...
  async function generateLabel(id) {
    alert("Agora a etiqueta deve ser gerada manualmente no Melhor Envio. Depois, cole o código de rastreio neste pedido e mude o status para Enviado.");
  }
...
function App() {
  ...
  <b>MBLab</b>
  <span>
    Loja oficial de livros • suporte:{" "}
    <a href="mailto:mateusbuarquepugli@gmail.com">
