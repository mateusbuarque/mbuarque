...
function stateFromCep(cep) {
  const clean = String(cep || "").replace(/\D/g, "");
  if (clean.length !== 8) return "";

  const prefix = Number(clean.slice(0, 5));
...
function calculateShipping(cep, state, total) {
  ...
  // O CEP digitado define a região do frete. O estado salvo na conta é apenas fallback.
  const uf = stateFromCep(clean) || String(state || "").toUpperCase().trim();
...
function Header({ customer, customerLogout, cartCount, favoriteCount, adminLogged = false }) {
      ...
      <header>
        ...
        <nav className={open ? "open" : ""}>
          ...
          {adminLogged
            ...
            : customer
              ? <button onClick={() => { customerLogout(); closeMenu(); }}>Sair cliente</button>
...
function BookPage({ data, cart, customer, wishlist, notify, load }) {
  ...
  useEffect(() => {
    ...
    api("/.netlify/functions/store-data?action=customer-orders", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: customer.email, password: customer.password })
    }).then(response => setCustomerOrders(response.orders || [])).catch(() => setCustomerOrders([]));
  ...
  const stock = stockLabel(book);
  const summary = ratingSummary(reviews);
  const hasPurchased = Boolean(customer && customerOrders.some(order =>
...
function Login({ data, customerSave, adminLogin, notify }) {
  ...
  async function submit(event) {
    ...
    if (mode === "login") {
      try {
        const response = await api("/.netlify/functions/store-data?action=customer-login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, password: form.password })
        });
        customerSave(response.customer);
        notify("Login realizado com sucesso.");
        location.hash = "/loja";
        return;
      } catch {
        const isAdmin = await adminLogin(email, form.password, { silentError: true });
        if (isAdmin) return;
        notify("Login incorreto ou cadastro não encontrado.", "error");
        return;
      }
    }
...
function Cart({ data, cart, customer, notify }) {
  ...
  async function pay() {
    ...
    try {
      ...
      const { password: _customerPassword, ...customerWithoutPassword } = customer;
      const buyer = { ...customerWithoutPassword, cep: shippingCep || customer.cep };
...
function MyPurchases({ customer, notify }) {
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const response = await api("/.netlify/functions/store-data?action=customer-orders", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: customer.email, password: customer.password })
      });
...
function MyAccount({ customer, customerLogout, cartCount, favoriteCount }) {
  if (!customer) {
    location.hash = "/login";
    return null;
  }
  ...
  return (
    <Section title="Minha conta">
      <div className="accountWelcome">
        <div>
          <span>Olá,</span>
          <h2>{customer.name || "Cliente MBLab"}</h2>
          <p>{customer.email}</p>
        </div>
        <button className="btn" onClick={customerLogout}>Sair da conta</button>
      </div>

      <div className="accountGrid">
        <a className="accountCard" href="#/favoritos">
          <Heart size={28} />
          <div>
            <strong>Favoritos</strong>
            <span>{favoriteCount} {favoriteCount === 1 ? "livro salvo" : "livros salvos"}</span>
          </div>
        </a>

        <a className="accountCard" href="#/minhas-compras">
          <CheckCircle2 size={28} />
          <div>
            <strong>Minhas compras</strong>
            <span>Acompanhe pedidos, status e rastreio</span>
          </div>
        </a>

        <a className="accountCard" href="#/carrinho">
      ...
      <div className="accountDetails box">
        <h2>Meus dados</h2>
...
function App() {
  ...
  useEffect(() => {
    api("/.netlify/functions/admin-auth")
      .then(async response => {
        const authenticated = Boolean(response.authenticated);
        setAdminLogged(authenticated);
        if (authenticated) await load({ silent: true });
      })
      .catch(() => setAdminLogged(false));
  }, []);

  async function adminLogin(email, password, { silentError = false } = {}) {
    try {
      const response = await api("/.netlify/functions/admin-auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "login", email, password })
      });
      if (!response.authenticated) throw new Error("Login incorreto.");
      setAdminLogged(true);
      await load({ silent: true });
      ...
      const message = error.message || "Login incorreto.";
      const configurationError = /configura|ADMIN_|Netlify|deploy/i.test(message);
      if (!silentError || configurationError) notifications.notify(message, "error");
      return false;
    }
  }
  ...
  async function adminLogout() {
    try {
      await api("/.netlify/functions/admin-auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  else if (route === "/minha-conta") page = <MyAccount customer={customer} customerLogout={customerLogout} cartCount={cart.cart.reduce((total, item) => total + Number(item.qty || 1), 0)} favoriteCount={wishlist.ids.length} />;
  ...
  return (
    <>
      <Header customer={customer} customerLogout={customerLogout} cartCount={cart.cart.reduce((total, item) => total + Number(item.qty || 1), 0)} favoriteCount={wishlist.ids.length} adminLogged={adminLogged} />
      {page}
      <footer>
        <b>MBLab</b>
