...
function Header({ customer, customerLogout, cartCount, favoriteCount, adminMode = false }) {
  const [open, setOpen] = useState(false);
  const publicLinkProps = adminMode ? { target: "_blank", rel: "noreferrer" } : {};
  ...
  return (
    <>
      <header>
        ...
        <a className="brand" href="#/" onClick={closeMenu} {...publicLinkProps}><img src="/logo.jpeg" alt="MBLab" />MBLab</a>
        <nav className={open ? "open" : ""}>
          <a href="#/" onClick={closeMenu} {...publicLinkProps}>Início</a>
          <a href="#/loja" onClick={closeMenu} {...publicLinkProps}>Livros</a>
          <a href="#/favoritos" onClick={closeMenu} {...publicLinkProps}>Favoritos ({favoriteCount})</a>
          <a href="#/carrinho" onClick={closeMenu} {...publicLinkProps}>Carrinho ({cartCount})</a>
          <a href="#/minhas-compras" onClick={closeMenu} {...publicLinkProps}>Minhas compras</a>
          <a href="#/minha-conta" onClick={closeMenu} {...publicLinkProps}>Minha conta</a>
          {customer ? <button onClick={() => { customerLogout(); closeMenu(); }}>Sair cliente</button> : <a href="#/login" onClick={closeMenu} {...publicLinkProps}>Entrar</a>}
        </nav>
      </header>

      <div className="cat">
        <a href="#/loja" {...publicLinkProps}>Todos</a>
        <a href="#/loja/fisicos" {...publicLinkProps}>Livros</a>
        <a href="#/loja/promocoes" {...publicLinkProps}>Promoções</a>
      </div>

      <div className="mobileBottomNav">
        <a href="#/" {...publicLinkProps}>Início</a>
        <a href="#/loja" {...publicLinkProps}>Livros</a>
        <a href="#/favoritos" {...publicLinkProps}>Favoritos</a>
        <a href="#/carrinho" {...publicLinkProps}>Carrinho{cartCount ? ` (${cartCount})` : ""}</a>
        <a href="#/minha-conta" {...publicLinkProps}>Conta</a>
      </div>
    </>
  );
}
...
function App() {
  ...
  return (
    <>
      <Header customer={customer} customerLogout={customerLogout} cartCount={cart.cart.reduce((total, item) => total + Number(item.qty || 1), 0)} favoriteCount={wishlist.ids.length} adminMode={route === "/admin" && adminLogged} />
      {page}
      <footer>
        <b>MBLab</b>
