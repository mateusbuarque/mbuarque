...
function createLabelCode(orderId) {
  return `MBL${String(orderId).replace(/\D/g, '').slice(-6).padStart(6, '0')}${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
}

function ShippingLabel({ order, save }) {
  const code = order.shipping_label_code || createLabelCode(order.id);
  useEffect(() => {
    if (!order.shipping_label_code) {
      save(order.id, { shipping_label_code: code, label_created_at: new Date().toISOString() });
    }
  }, []);

  return <div className="thermalLabel">
    <h2>MBLab</h2>
    <b>ETIQUETA DE ENVIO</b>
    <hr/>
    <strong>PEDIDO #{order.id}</strong>
    <div className="barcode"><span>{code}</span></div>
    <hr/>
    <b>DESTINATÁRIO</b>
    <p>{order.buyer?.name}</p>
    <p>{order.buyer?.street}, {order.buyer?.number}</p>
    <p>{order.buyer?.complement}</p>
    <p>{order.buyer?.neighborhood}</p>
    <p>{order.buyer?.city}/{order.buyer?.state}</p>
    <p>CEP: {order.buyer?.cep}</p>
    <p>Tel: {order.buyer?.phone}</p>
    <hr/>
    <b>REMETENTE</b>
    <p>MBLab<br/>Osasco - SP</p>
    <button className="printHide btn red" onClick={() => window.print()}>Imprimir</button>
  </div>;
}
...
function ShippingLabelPage({ data }) {
  ...
  async function save(id, body) {
    await api(`/.netlify/functions/store-data?table=orders&id=${encodeURIComponent(id)}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)});
  }
  if (!order) return <Section title="Etiqueta">Pedido não encontrado.</Section>;
  return <ShippingLabel order={order} save={save}/>;
}
