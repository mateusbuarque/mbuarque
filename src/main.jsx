...
function ShippingLabel({ order, save }) {
  const [code] = useState(() => order.shipping_label_code || createLabelCode(order.id));
  const buyer = order.buyer || {};
  const service = order.shipping?.name || order.shipping_service || "PAC / SEDEX";
  ...
  useEffect(() => {
    if (!order.shipping_label_code) {
      save(order.id, { shipping_label_code: code, label_created_at: new Date().toISOString() }).catch(() => {});
    }
  }, []);
  ...
  return (
    <article className="shippingLabelSheet">
      ...
      <section className="shippingLabelSummary">
        ...
        <div><b>RASTREIO:</b><span>{tracking || "Não informado"}</span></div>
      ...
      <section className="shippingLabelRecipient">
        <h2>DESTINATÁRIO</h2>
        <dl>
          <div><dt>Nome:</dt><dd>{buyer.name || "Não informado"}</dd></div>
          <div><dt>Endereço:</dt><dd>{address || "Não informado"}</dd></div>
          <div><dt>Número / complemento:</dt><dd>{[number, complement].filter(Boolean).join(" - ") || "Não informado"}</dd></div>
          <div><dt>Bairro:</dt><dd>{neighborhood || "Não informado"}</dd></div>
          <div><dt>Cidade / UF:</dt><dd>{[city, state].filter(Boolean).join(" / ") || "Não informado"}</dd></div>
          <div><dt>CEP:</dt><dd>{cep || "Não informado"}</dd></div>
          <div><dt>Telefone:</dt><dd>{phone || "Não informado"}</dd></div>
        </dl>
      </section>
