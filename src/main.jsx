...
function calculateCouponDiscount(items, coupon) {
  ...
  if (coupon.type === "percent") {
    return { valid: true, discount: Number((subtotal * Number(coupon.value || 0) / 100).toFixed(2)), message: "" };
  }

  return { valid: true, discount: Math.min(subtotal, Number(coupon.value || 0)), message: "" };
}
...
function CouponsAdmin({ data, load, notify }) {
  const emptyForm = { code: "", type: "percent", value: 10, buy_quantity: 2, pay_quantity: 1, active: true };
        ...
        <div className="list">
          {data.coupons.map(coupon => (
            <div className="row" key={coupon.id}>
              <span><b>{coupon.code}</b><small>{couponDescription(coupon)} • {coupon.active ? "Ativo" : "Inativo"}</small></span>
