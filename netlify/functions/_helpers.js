...
async function getSetting(key) {
  const data = await supabase(`app_settings?key=eq.${encodeURIComponent(key)}&select=value`, { method: "GET" }).catch(() => []);
...
async function getMelhorEnvioToken() {
  const fixedToken = String(process.env.MELHOR_ENVIO_TOKEN || "").trim();
  if (fixedToken) return fixedToken.replace(/^Bearer\s+/i, "");
  if (process.env.MELHOR_ENVIO_TOKEN) return process.env.MELHOR_ENVIO_TOKEN;
  const saved = await getSetting("melhor_envio_oauth");

  if (saved?.access_token && saved?.expires_at && Number(saved.expires_at) > Date.now()) {
    return saved.access_token;
  }

  if (saved?.refresh_token) {
    return exchangeMelhorEnvioToken({ grantType: "refresh_token", refreshToken: saved.refresh_token });
  }

  const code = process.env.MELHOR_ENVIO_AUTH_CODE;
  if (!code) throw new Error("Configure MELHOR_ENVIO_AUTH_CODE no Netlify.");
